...
<meta name="description" content="Laporan Nilai | Sistem Informasi Penilaian Akademik Kurikulum 2013 Berbasis Web Pada SDN Tangkil 03 Wlingi - Blitar">
<meta name="author" content="Malik">
<title>Laporan Nilai | Sistem Informasi Penilaian Akademik Kurikulum 2013 Berbasis Web Pada SDN Tangkil 03 Wlingi - Blitar</title>
...
<div class="container" style="margin-top:70px; width:95%;">
  <div class="row">
    <div class="" >
            ...
            <div class="panel-heading">Laporan Nilai</div>
                    
              
                        <div class="panel-body" >
            		<form action="laporan-nilai.php" method="GET" enctype="multipart/form-data" class="form-horizontal">
              ...
              <div class="col-sm-2">
                ...
                <button type="submit" name="cari" class="btn btn-sm btn-primary"><span class="glyphicon glyphicon-search"></span> Proses</button>
                 ...
                 <div class="alert alert-danger">
                    <b>Keterangan :</b> Pilih Kelas, Semester & Tahun Ajaran terlebih dahulu untuk melihat laporan nilai.
                 </div> 
 ...
 <?php 
              $query2 = mysqli_query($koneksi,"SELECT * FROM nilai,siswa WHERE nilai.nis=siswa.nis and nilai.id_kelas='$_GET[id_kelas]' and nilai.semester='$_GET[semester]' and nilai.tahun_ajaran='$_GET[thn]'");
              $cari = mysqli_num_rows($query2);
              if($cari == 0){
				echo "<tr>
            <td colspan='24' style='border: none;'><center><b>Data Tidak Di Temukan / Guru Kelas Belum Memberikan Nilai</a></center><br></td>
                        </tr>";
              }else{
			  
              $query22 = mysqli_query($koneksi,"SELECT nilai.nis,siswa.nama_siswa,nilai.id_kelas FROM nilai,siswa WHERE nilai.nis=siswa.nis and nilai.id_kelas='$_GET[id_kelas]' and nilai.semester='$_GET[semester]' and nilai.tahun_ajaran='$_GET[thn]' group by nilai.nis");
              while($data22 = mysqli_fetch_array($query22)){
				  ?>
                  		<tr>
                       		<th colspan='24'>
                        <center><font color="#009933"><?php echo $data22['nis']." | ".$data22['nama_siswa']; ?></font></center>
                        	</th>
                        </tr>
                  <?php
				  	$no = 0;
					$tr = 0;
				  	$query2 = mysqli_query($koneksi,"SELECT * FROM nilai,siswa WHERE nilai.nis=siswa.nis and nilai.id_kelas='$_GET[id_kelas]' and nilai.semester='$_GET[semester]' and nilai.tahun_ajaran='$_GET[thn]' and nilai.nis='".$data22['nis']."'");
...
toleransi+gt_royong+santun+percaya_diri)/20 as rata2 FROM nilai WHERE nis = '".$data22['nis']."'");
...
<?php
if($dr['rata2']>=80 and $dr['rata2']<=100){ echo "Ananda $data22[nama_siswa] terus belajar, penjelasan sesuai predikat (A)"; }
else if($dr['rata2']>=70 and $dr['rata2']<80){ echo "Ananda $data22[nama_siswa] terus belajar, penjelasan sesuai predikat (B)"; }
else if($dr['rata2']>=60 and $dr['rata2']<70){ echo "Ananda $data22[nama_siswa] terus belajar, penjelasan sesuai predikat (C)"; }
else if($dr['rata2']>=50 and $dr['rata2']<60){ echo "Ananda $data22[nama_siswa] terus belajar, penjelasan sesuai predikat (D)"; }
else if($dr['rata2']>=0 and $dr['rata2']<50){ echo "Ananda $data22[nama_siswa] terus belajar, penjelasan sesuai predikat (E)"; }
?> 
      </div>   
...
if($trr>=80 and $trr<=100){ echo "<font color='#FF6600'> (A) </font>"; }
 ...
 ?>
<a href="#modalrt<?php echo $data22['nis']; ?>" data-toggle="modal">diskripsi</a>

<!-- Modal Popup untuk catatan--> 
<div class="modal fade" id="modalrt<?php echo $data22['nis']; ?>">
  <div class="modal-dialog">
    <div class="modal-content" style="margin-top:100px;">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
      ...
      <div class="modal-body">
<?php
if($trr>=80 and $trr<=100){ echo "Ananda $data22[nama_siswa] terus belajar, penjelasan sesuai predikat (A)"; }
else if($trr>=70 and $trr<80){ echo "Ananda $data22[nama_siswa] terus belajar, penjelasan sesuai predikat (B)"; }
else if($trr>=60 and $trr<70){ echo "Ananda $data22[nama_siswa] terus belajar, penjelasan sesuai predikat (C)"; }
else if($trr>=50 and $trr<60){ echo "Ananda $data22[nama_siswa] terus belajar, penjelasan sesuai predikat (D)"; }
else if($trr>=0 and $trr<50){ echo "Ananda $data22[nama_siswa] terus belajar, penjelasan sesuai predikat (E)"; }
?> 
      </div>   
      <div class="modal-footer" style="margin:0px; border-top:0px; text-align:center;">
        <button type="button" class="btn btn-primary" data-dismiss="modal">close</button>
                        ...
                        <tr><td colspan="24"><a class="btn btn-sm btn-info" href="lap-nilai-excel.php?id_kelas=<?php echo $data22['id_kelas']; ?>&semester=<?php echo $semester; ?>&cari=&nis=<?php echo $data22['nis']?>&thn=<?php echo $_GET['thn']?>">Export To Excel</a>
                        <a class="btn btn-sm btn-success" href="cetak-rapor.php?id_kelas=<?php echo $data22['id_kelas']; ?>&semester=<?php echo $semester; ?>&cari=&nis=<?php echo $data22['nis']?>&thn=<?php echo $_GET['thn']?>">Cetak Rapor</a>
                        </td></tr>
                    <?php } } ?>                      
                    </table>
                    <?php } ?>
